...
#
# 这一步同时要验**两条通道**：明文那份配置，和 HTTPS 那份（`deploy/nginx/tls`，
# 用自签证书真起 TLS）。每条通道都配了反例 —— 只验正例的话，
# 一把恒真的尺子也能给出「通过」：
#   · 明文 A/B：开着 proxy_buffering、上游换成靠关连接结束的 tests/e2e/plain_sse.py，
#     必须被判成攒批；
#   · HTTPS C/D：默认信任链打自签证书必须验签失败（证明校验真的开着）、
#     TLS 下开着 buffering 也必须被判成攒批（证明尺子在 TLS 上不瞎）。
