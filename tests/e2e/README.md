...
`nginx` 那一组的性质特殊：它守的三类文件（明文模板 / HTTPS 模板 / compose 接线）
**错了本机不会有任何业务症状** —— 静态校验只能证明文本写对了。真要证明「规则生效」，
得让 `nginx_check.py` 在一台有 Docker 的机器上真起一个 nginx 量到达时刻；这件事
**不在主 CI 里**，只在 `container-smoke.yml` 第 9 步做。
